  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <Card className="lg:col-span-2">
        <h2 className="text-lg font-bold text-slate-900">1. Upload satellite image</h2>
        <p className="mt-1 text-sm text-slate-500">Choose an analysis type, then drop one or two images.</p>
                ...
                {img ? (
                  <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="max-w-[200px] truncate text-sm font-medium text-slate-800">{img.name}</div>
                  ...
                  <label
                    ...
                    className={`flex h-28 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-3 text-center transition ${
                      dragging ? 'border-primary bg-primary-50' : 'border-slate-300 bg-slate-50 hover:border-primary/50'
                    }`}
                  >
                    <span className="text-2xl text-primary">⬆</span>
                    <span className="mt-1 text-sm font-medium text-slate-700">Drop image here</span>
                    <span className="text-xs text-slate-400">or click to browse · GeoTIFF/TIFF, PNG, JPEG</span>
                    <input
                      type="file"
        ...
        <div className="mt-5 flex items-center justify-between">
          <p className="text-xs text-slate-400">{`Files: ${images.length}/${required}`}</p>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-slate-600">Or run a demo</h2>
        {DEMO_SCENARIOS.map((s) => (
          <button
            key={s.id}
            onClick={() => onRunScenario(s)}
            className="w-full rounded-xl bg-white p-4 text-left shadow-sm ring-1 ring-slate-200 transition hover:ring-primary/50"
          >
        ...
        <p className="text-xs text-slate-400">Demo scenarios play through the full flow with pre-generated results.</p>
      </div>
    </div>
  )
}