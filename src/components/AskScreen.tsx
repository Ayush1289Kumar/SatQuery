    ...
    <div className="mx-auto max-w-3xl">
      <Card glow="violet">
        ...
        <div className="mb-6 flex items-center gap-3">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[rgba(139,92,246,0.18)] border border-[rgba(139,92,246,0.30)]">
          ...
          {images.map((img, i) => (
            ...
            >
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                img.kind === 'sar'
                  ? 'bg-[rgba(34,211,238,0.18)] text-[#22d3ee]'
                  : 'bg-[rgba(59,130,246,0.18)] text-[#3b82f6]'
              }`}>
                {i + 1}
              </span>
              <span className="max-w-[160px] truncate">{img.name}</span>
              <span className={`text-[10px] ${img.kind === 'sar' ? 'text-[#22d3ee]/60' : 'text-[#3b82f6]/60'}`}>
        ...
        <div className="relative mt-5">
          <textarea
            ...
            className="w-full resize-none rounded-xl border border-[rgba(255,255,255,0.10)] bg-[rgba(255,255,255,0.04)] p-4 text-sm text-[rgba(255,255,255,0.85)] placeholder:text-[rgba(255,255,255,0.25)] transition-all duration-200 focus:border-[rgba(139,92,246,0.50)] focus:bg-[rgba(139,92,246,0.05)] focus:outline-none focus:shadow-[0_0_20px_-6px_rgba(139,92,246,0.45)]"
          />
          {question.trim() && (
          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {SUGGESTED_QUESTIONS.map((q) => (
              <button
                ...
                className="rounded-full border border-[rgba(139,92,246,0.25)] bg-[rgba(139,92,246,0.08)] px-3.5 py-1.5 text-xs font-medium text-[#8b5cf6] transition-all duration-150 ease-out hover:border-[rgba(139,92,246,0.45)] hover:bg-[rgba(139,92,246,0.15)] hover:shadow-[0_0_12px_-4px_rgba(139,92,246,0.50)] active:scale-[0.97]"
              >
                {q.label}
              </button>
            ))}
